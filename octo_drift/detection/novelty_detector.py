...
from ..core.fuzzy_functions import FuzzyCMeans, fuzzy_silhouette, create_spfmics_from_clusters
...
class NoveltyDetector:
    ...
    def __init__(self, k_short: int, phi: float, min_weight: int,
                 alpha: float, theta: float, fuzzification: float):
    ...
    def detect(self, unknown_buffer: List[Example], 
               supervised_model: SupervisedModel,
               unsupervised_model: UnsupervisedModel,
               timestamp: int) -> Tuple[List[Example], bool]:
        """
        ...
        valid_clusters = [
            i for i in range(len(silhouettes))
            if silhouettes[i] > 0 and 
               len(clusterer.get_cluster_members(i, unknown_buffer)) >= self.min_weight
        ]
        ...
        for cluster_idx in valid_clusters:
            ...
            if not overlap_factors:
                ...
                self._process_as_novelty(
                    candidate, cluster_members, unknown_buffer, 
                    unsupervised_model, timestamp
                )
            ...
            if min_overlap <= self.phi:
                ...
                self._process_as_extension(
                    candidate, cluster_members, unknown_buffer,
                    unsupervised_model, known_label, timestamp
                )
            else:
                # True novelty
                self._process_as_novelty(
    ...
    def _process_as_extension(self, candidate: SPFMiC, members: List[Example],
        ...
        # Find most frequent true label in cluster
