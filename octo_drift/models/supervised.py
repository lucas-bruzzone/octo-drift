...
class SupervisedModel:
    ...
    def __init__(
        self, k: int, fuzzification: float, alpha: float, theta: float, min_weight: int
    ):
        self.k = k
        ...
        for spfmic in all_spfmics:
            ...
            if distance <= spfmic.radius_weighted:
                typicality = spfmic.calculate_typicality(
                    example.point, self.k, distance
                )
                pertinence = calculate_pertinence(
                    example.point, spfmic.centroid, self.fuzzification
                )
    ...
    def train_new_classifier(
        self, examples: List[Example], timestamp: int
    ) -> List[Example]:
        """
