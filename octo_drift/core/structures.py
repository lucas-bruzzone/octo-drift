...
class Example:
    ...
    @classmethod
    def from_array(cls, data: npt.NDArray[np.float64], 
                   has_label: bool = True, 
                   timestamp: int = 0) -> 'Example':
        """
...
class SPFMiC:
    ...
    created: int = 0
    updated: int = 0
    cf1_pertinence: npt.NDArray[np.float64] = field(default_factory=lambda: np.array([]))
    cf1_typicality: npt.NDArray[np.float64] = field(default_factory=lambda: np.array([]))
    me: float = 1.0
    te: float = 1.0
    ssde: float = 0.0
    ...
    def update_centroid(self) -> None:
        ...
        if denominator > 0:
            self.centroid = (
                self.alpha * self.cf1_pertinence + 
                self.theta * self.cf1_typicality
            ) / denominator
    
    def assign_example(self, example: Example, 
                       pertinence: float, 
                       typicality: float,
                       distance: float) -> None:
        """
    ...
    def calculate_typicality(self, point: npt.NDArray[np.float64], 
                            k: int,
