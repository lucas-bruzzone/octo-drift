...
class FuzzyCMeans:
    ...
    def __init__(self, n_clusters: int, fuzzification: float = 2.0,
    ...
    def get_cluster_members(self, cluster_idx: int, 
...
def calculate_typicality_matrix(membership_matrix: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
    """
...
def fuzzy_silhouette(clusterer: FuzzyCMeans,
...
def create_spfmics_from_clusters(examples: List[Example],
                                 clusterer: FuzzyCMeans,
                                 label: float,
                                 alpha: float,
                                 theta: float,
                                 min_weight: int,
                                 timestamp: int) -> List[SPFMiC]:
    """
    Create SPFMiC micro-clusters from fuzzy clustering results.
    
    Args:
        examples: List of examples
        clusterer: Fitted FuzzyCMeans
        label: Class label for these micro-clusters
    ...
    for cluster_idx in range(clusterer.n_clusters):
        ...
        for i, example in enumerate(examples):
            example_idx = examples.index(example)
